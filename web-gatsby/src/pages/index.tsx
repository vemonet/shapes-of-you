...
import { Typography, Container, Box, Button, Chip, Alert, Grid, Paper, CircularProgress, Card, CardContent, CardHeader, Collapse, CardActions } from "@mui/material";
...
import SwaggerIcon from '@mui/icons-material/SettingsEthernet';
...
export default function SemanticIndex() {
  ...
  const getShapesFiles = (sparqlQuery: string) => {

    const searchQuery = `PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
    ...
    SELECT DISTINCT * WHERE {
      GRAPH ?graph {
        ...
        OPTIONAL {
          ?file_url schema:query ?query .
          FILTER (strlen(str(?query)) > 1)
        }
        OPTIONAL { ?file_url void:sparqlEndpoint ?sparql_endpoint }
        OPTIONAL { ?file_url rdfs:comment ?file_description }
      }
    } LIMIT ${100000}`

    // Search for files
    axios.get(endpointToQuery + `?query=` + encodeURIComponent(searchQuery))
      .then(res => {
        ...
        sparqlResultArray.map((result: any): any =>  {
          ...
          const repo_url = result.repo_url.value;
          ...
          let file_type = 'http://www.w3.org/ns/shacl#SPARQLFunction'
          if (result.file_type) {
            file_type = result.file_type.value;
          }

          // Create shape file entry
          const file_url = result.file_url.value;
          // Avoid duplicates
          ...
          if (!found) {
            let file_obj: any = {
              'url': file_url,
              'type': file_type,
              'label': result.file_label.value,
            }
            if (result.file_description) {
              file_obj.description = result.file_description.value;
            }
            if (result.sparql_endpoint) {
              file_obj.sparqlEndpoint = result.sparql_endpoint.value;
            }
            if (result.query) {
  ...
  React.useEffect(() => {
    ...
    getShapesFiles(queryGetSparqlQueries)
